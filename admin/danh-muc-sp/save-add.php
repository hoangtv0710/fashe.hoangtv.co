<?php 
	...
	if(!checkXss($name)){
		header('location: '. SITELINKADMIN . '/danh-muc-sp/add.php?errName=Tên danh mục không hợp lệ!&name='.$name.'&description='.$description);
		die;
	}

	if(!checkXss($description)){
		header('location: '. SITELINKADMIN . '/danh-muc-sp/add.php?errDescription=Nội dung danh mục không hợp lệ!&name='.$name.'&description='.$description);
		die;
	}
